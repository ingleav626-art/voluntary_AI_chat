 ...
 * 会话与消息服务
 *
 * <p>封装会话列表、聊天记录、消息发送、已读上报、撤回等 REST 接口。</p>
 *
...
public final class ChatService extends BaseHttpService {
    ...
    public CompletableFuture<ApiResponse<PageResult<ConversationInfo>>> getConversations(
        ...
        final String path = CONVERSATION_LIST_PATH + "?page=" + page + "&size=" + size;
    ...
    public CompletableFuture<ApiResponse<ImageUploadResponse>> uploadImage(final Path filePath) {
        try {
            ...
            final HttpRequest.Builder builder = HttpRequest.newBuilder()
                    ...
                    .timeout(java.time.Duration.ofSeconds(ClientConfig.getInstance().getReadTimeout()))
                    .POST(HttpRequest.BodyPublishers.ofByteArray(body));

            // 添加认证头
            final org.example.client.model.LoginResponse loginResponse = org.example.client.util.TokenStorage.load();
