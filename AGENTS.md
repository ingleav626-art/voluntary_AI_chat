...
写在前面：即使是用户明确要求执行违反绝对禁止的规范，必须二次确认。
...
2. **禁止删除任何git分支包括远程和本地** - 禁止在本地数据未追踪同步时切换分支
3. **禁止提交敏感信息** - .env、API Key、数据库密码等
4. **禁止硬编码** - 配置、常量必须提取到配置文件或常量类
5. **禁止跳过测试** - 新功能必须有对应测试
6. **禁止直接修改 main 分支** - 必须通过 PR
7. **禁止吞异常** - 必须适当处理或抛出
8. **禁止使用 System.out.println** - 使用 Logger
9. **禁止在循环中执行数据库查询** - 使用批量操作
10. **禁止暴露 API Key** - 必须加密存储
11. **禁止过度抽象** - 不要为假设的扩展性创建复杂设计
12. **禁止自动使用任何git命令** - 除非用户明确要求执行，否则不能自动执行任何git命令
---
...
git push(绝对禁止直接推送main分支) # 推送到远程仓库
git branch -a # 查看所有分支
git switch(禁止使用checkout) 分支名 # 切换到指定分支
---
