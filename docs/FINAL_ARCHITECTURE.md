...
### 当前实现状态（2026-06-27 更新）

三包架构已全部实现，L1/L2 功能已完成，L3 功能为云端演进规划：

| 层级 | 状态 | 说明 |
|------|------|------|
| **L1（开箱即用）** | 已实现 | 客户端包 LocalAiEngine POJO + H2、测试包嵌入式 Spring Boot、云端包 Fat JAR |
| **L2（填配置）** | 已实现 | 云端包配置 MySQL/Redis 环境变量即可运行 |
| **L3（下载依赖）** | 规划中 | MinIO（分布式文件存储）、Milvus/Qdrant（向量检索）、RabbitMQ（消息队列）、DJL（本地模型）——均为云端多用户场景的演进方向，当前使用轻量替代方案 |

> 文档中对 MinIO/Milvus/RabbitMQ/DJL 的描述代表**技术路线图**，当前实现使用：本地文件存储、MySQL 关键词匹配、ScheduledExecutorService、直接调用远程 AI。
...
| 包 | 内容 | 启动 | 大小 | 状态 |
|----|------|------|------|------|
| 客户包 | JavaFX + LocalAiEngine POJO（无 Spring Boot） | <1s | ~44MB | 已实现 |
| 测试包 | JavaFX + 内嵌 Spring Boot (server全量) | ~7s | ~84MB | 已实现 |
| 云端包 | Spring Boot fat JAR | ~3s | ~49MB | 已实现 |

> **更新（2026-06-27）**：客户包已通过 `LocalAiEngine` POJO 模式实现无 Spring Boot 启动。LOCAL 模式下检测 classpath 是否有 server 模块，无则使用 LocalAiEngine 直连 H2。
